...
app.get('/', (req, res) => {
  res.json({ 
    status: 'ok', 
    message: 'BestDeal API - Real Scraping',
    version: '2.0.0',
    endpoints: {
      health: '/health',
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      message: `Scraped ${amazonProducts.length} from Amazon, ${flipkartProducts.length} from Flipkart`
    });
  } catch (error) {
...
// Get product prices
app.get('/api/product/:productId/prices', async (req, res) => {
  try {
